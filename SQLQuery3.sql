...
/*SELECT DateDiff(mi, started_at, ended_at) AS "Time difference",
	started_at AS "Start", 
	ended_at AS "end"
FROM [bike_data].[dbo].[bike_data2];
DESC [bike_data].[dbo].[bike_data2]; */

/*SELECT AVG(CAST(DateDiff(mi, started_at, ended_at) as DECIMAL(7,2))) AS "Average ride_length",
day_of_week
	FROM [bike_data].[dbo].[bike_data2]
	GROUP BY day_of_week; */
